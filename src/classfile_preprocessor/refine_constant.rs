...
pub fn refine_constant(raw_constant : &RawCpInfo) -> Constant {
	...
	match raw_constant.tag {
		...
		8 => Constant::String {string_index : 
			read_u16(bytes[0], 
				bytes[1])
		},
		3 => Constant::Integer {
			bytes : read_u32(bytes[0], 
				bytes[1],
				bytes[2],
				bytes[3])
		},
		...
		5 => Constant::Long {
				low_bytes : read_u32(bytes[0], 
					bytes[1],
					bytes[2],
					bytes[3]),
				high_bytes : read_u32(bytes[4], 
					bytes[5],
					bytes[6],
					bytes[7])
		},
		...
		12 => Constant::NameAndType {
			name_index : read_u16(bytes[0], 
					bytes[1]),
			descriptor_index : read_u16(bytes[2],
					bytes[3])
		},
		...
		16 => Constant::MethodType {
			descriptor_index : read_u16(bytes[0], bytes[1])
		},
...
fn make_methodhandle(reference_kind : u8, reference_index : u16) -> Constant {
	Constant::MethodHandle {
		...
		reference_kind : match reference_kind {
			...
			_ => panic!("Invalid method handle reference kind was {}", reference_kind)
		}
	}
}