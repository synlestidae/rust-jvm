...
pub fn refine_classfile(raw_classfile: &RawClassFile)
    ...
    if let (&Constant::Class {name_index : this_class_name_index},
            &Constant::Class {name_index : super_class_name_index}) = this_and_super {
    ...
    let result = RefinedClassFile {
        ...
        field_table: process_field_table(&constants, &raw_classfile.field_table),
        method_table: process_method_table(&constants, &raw_classfile.method_table),
        attribute_table: process_attributes(&constants, &raw_classfile.attribute_table),
    };
...
fn process_method_table(constants: &Vec<Constant>, table: &Vec<RawInfo>) -> Vec<Info> {
    table.iter().map(|ref raw_method_info| refine_info(constants, &raw_method_info)).collect()
}

fn process_field_table(constants: &Vec<Constant>, table: &Vec<RawInfo>) -> Vec<Info> {
    table.iter().map(|ref raw_field_info| refine_info(constants, &raw_field_info)).collect()
}

fn process_attributes(constants: &Vec<Constant>, table: &Vec<RawAttributeInfo>) -> Vec<Attribute> {
    table.iter()
         .map(|ref raw_attribute_info| refine_attribute(constants, raw_attribute_info))
         .collect()
}
...
fn refine_info(constants: &Vec<Constant>, raw_info: &RawInfo) -> Info {
    ...
    match (name_constant, descriptor_constant) {
        (&Constant::Utf8(ref name), &Constant::Utf8(ref descriptor)) => {
            Info {
