    ...
    match name {
        ...
        "Code" => {
            ...
            let exception_table_length =
                read_u16(bytes[code_length + 8], bytes[code_length + 9]) as usize;
            ...
            j += 2;
            let attributes = read_attributes_info(bytes,
                                                  &mut (j as usize),
        ...
        "Exceptions" => {
            ...
            Attribute::Exceptions { exception_table: exceptions_table }
        }
        "InnerClasses" => {
            ...
            for i in 0..number_of_classes {
                ...
                classes.push((inner_class_info_index,
                              outer_class_info_index,
                              inner_name_index,
                              inner_class_access_flags));
            }
            Attribute::InnerClasses { classes: classes }
        }
        "EnclosingMethod" => {
        ...
        "Signature" => {
            ...
            match &constants[signature_index as usize - 1] {
                &Constant::Utf8(ref sig_string) => {
                    Attribute::Signature { signature: sig_string.clone() }
                }
                invalidConst => panic!("Unknown signature type: {:?}", invalidConst),
            }
        }
        "SourceFile" => {
            let sourcefile_index = read_u16(bytes[0], bytes[1]);
            match &constants[sourcefile_index as usize - 1] {
                &Constant::Utf8(ref source_string) => {
                    Attribute::SourceFile { sourcefile: source_string.clone() }
                }
                invalidConst => panic!("Unknown sourcefile type: {:?}", invalidConst),
            }
        }
        "SourceDebugExtension" => {
            Attribute::SourceDebugExtension { debug_extension: bytes.clone() }
        }
        "LineNumberTable" => {
            ...
            Attribute::LineNumberTable { line_number_table: line_number_info }
        }
        "LocalVariableTable" => {
            let length_of_table = read_u16(bytes[0], bytes[1]) as usize;
            ...
            while i < length_of_table {
                ...
                local_variable_table.push((start_pc, length, name_index, descriptor_index, index));
            }

            Attribute::LocalVariableTable { local_variable_table: local_variable_table }
        }
        "LocalVariableTypeTable" => {
            ...
            while i < length_of_table {
                ...
                let index = read_u16(bytes[i + 8], bytes[i + 9]);
                local_variable_type_table.push((start_pc,
                                                length,
                                                name_index,
                                                signature_index,
                                                index));
...
fn read_stackmaptable_entries(constants: &Vec<Constant>,
                              index: &mut usize,
                              bytes: &Vec<u8>)
                              -> Vec<StackMapFrame> {
...
fn read_stackmaptable_entry(constants: &Vec<Constant>,
                            index: &mut usize,
                            bytes: &Vec<u8>)
                            -> StackMapFrame {
...
fn read_verification_type_info(constants: &Vec<Constant>,
                               index: &mut usize,
                               count: u16,
                               bytes: &Vec<u8>)
    ...
    *index += 1;
    match bytes[*index - 1] {
        ...
        7 => {
            let cpool_index = read_u16(bytes[*index], bytes[*index + 1]) as usize;
        ...
        8 => {
            ...
            *index += 2;
            VerificationTypeInfo::UninitializedVariable { offset: offset }
        }
        invalid_tag => {
            panic!("Invalid verification type info tag {} at {}",
                   invalid_tag,
                   (*index - 1))
        }
    }
